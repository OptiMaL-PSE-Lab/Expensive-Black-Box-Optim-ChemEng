...
def integrator_functionRand(pi, x0, xref, N, T):
    """Function which takes input pi - 1x3 array of k values- and integrates the system equation to return a system response.
    This function is called as many times as there are k triplets in the phi function input.
    """
    Dt = T/N
    ...
    for k in range(0, N): 
        ...
        if k < N:
            ...
            x_new = solve_ivp(sys_model,
                              ...
                              y0 = x[-1], 
                              args = ([u1, u2],))
            x_new = np.array(x_new["y"])[:,-1] + np.random.normal(0, 0.1, 2)
...
def phi_rand(pi, x0 = [15, 15], xref = [10, 10], N=200, T=3, return_sys_resp = False):
    """This function takes an array of k values stored in an nd array pi and returns the value of phi associated
        with each of the triplets of k within pi."""
    if type(pi) == list:
...
def reactor_sys_2st(t, x, u):
    ...
    dcAdt = (F_s + u[0])/V *(cA_in - cA) - 2*k_T*cA**2
    dTdt = (F_s + u[0])/V *(T_in + u[1] - T) + 2*(-deltaH_R/ro/cp)*k_T*cA**2 - UA/V/ro/cp*(T-Tj)
...
def integrator_reactor_2st(pi, x0, xref, N, T):
    ...
    for k in range(0, N): 
        ...
        u1 = max(min(k_vals[0]*e1 + k_vals[1]*sum(error1), 1), -1)
        u2 = max(min(k_vals[2]*e2 + k_vals[3]*sum(error2), 25), -25)
...
def integrator_reactor_2stexp(pi, x0, xref, N, T):
    """Function which takes input pi - 1x3 array of k values- and integrates the system equation to return a system response.
    This function is called as many times as there are k triplets in the phi function input.
    """
    Dt = T/N
...
def reactor_phi_2st(pi, x0 = [.6, 310], xref = [.666, 308.489], N=200, T=3, \
    ...
    """This function takes an array of k values stored in an nd array pi and returns the value of phi associated
        with each of the triplets of k within pi."""
    if type(pi) == list:
        phi_sol, system_response, control_out = integrator_reactor_2stexp(pi, x0, xref, N, T)
    elif isinstance(pi, np.ndarray):
        if pi.ndim == 1:
            phi_sol, system_response, control_out = integrator_reactor_2stexp(pi.tolist(), x0, xref, N, T)
        else:
            # pi = pi.reshape(3,).tolist()
