...
quadratic_pybobyqa2 = PyBobyqaWrapper().solve(Problem_quadratic, x0, bounds=bounds.T, \
                                      maxfun= max_f_eval, constraints=1)

N = 10
quadratic_Nest_list2 = []
for i in range(N):
    rnd_seed = i
    quadratic_Nest = nesterov_random(Problem_quadratic, x0, bounds, max_iter = 50, \
                          constraints = 1, rnd_seed = i, alpha = 1e-3, mu = 1e-2)
    quadratic_Nest_list2.append(quadratic_Nest)
print('10 Nesterov iterations completed')

N = 10
quadratic_simplex_list2 = []
for i in range(N):
    rnd_seed = i
    quadratic_simplex = simplex_method(Problem_quadratic, x0, bounds, max_iter = 50, \
                            constraints = 1, rnd_seed = i)
    quadratic_simplex_list2.append(quadratic_simplex)
print('10 simplex iterations completed')

quadratic_FiniteDiff2 = finite_Diff_Newton(Problem_quadratic, x0, bounds = bounds, \
                                   con_weight = 100)
    
quadratic_BFGS2 = BFGS_optimizer(Problem_quadratic, x0, bounds = bounds, \
                         con_weight = 100)
    
quadratic_Adam2 = Adam_optimizer(Problem_quadratic, x0, method = 'forward', \
...
for i in range(N):
    ...
    quadratic_CUATRO_global_list2.append(quadratic_CUATRO_global)
...
for i in range(N):
    ...
    quadratic_CUATRO_local1 = CUATRO(Problem_quadratic, x0, init_radius, bounds = bounds, \
                          ...
                          constr_handling = method)
    quadratic_CUATRO_local_list12.append(quadratic_CUATRO_local1)
print('10 CUATRO local iterations completed') 

N = 10
quadratic_Bayes_list2 = []
for i in range(1):
    Bayes = BayesOpt()
    pyro.set_rng_seed(i)
    ...
    quadratic_Bayes_list2.append(quadratic_Bayes)
 
print('10 BayesOpt iterations completed')

with open('BayesQuadraticTight_list.pickle', 'wb') as handle:
    pickle.dump(quadratic_Bayes_list2, handle, protocol=pickle.HIGHEST_PROTOCOL)



x_best_pyBbyqa = np.array(quadratic_pybobyqa2['x_best_so_far'])
f_best_pyBbyqa = np.array(quadratic_pybobyqa2['f_best_so_far'])

x_best_finDiff = np.array(quadratic_FiniteDiff2['x_best_so_far'])
f_best_finDiff = np.array(quadratic_FiniteDiff2['f_best_so_far'])
x_ind_findDiff = np.array(quadratic_FiniteDiff2['samples_at_iteration'])

x_best_BFGS = np.array(quadratic_BFGS2['x_best_so_far'])
f_best_BFGS = np.array(quadratic_BFGS2['f_best_so_far'])
x_ind_BFGS = np.array(quadratic_BFGS2['samples_at_iteration'])

x_best_Adam = np.array(quadratic_Adam2['x_best_so_far'])
f_best_Adam = np.array(quadratic_Adam2['f_best_so_far'])
x_ind_Adam = np.array(quadratic_Adam2['samples_at_iteration'])
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(oracle, bounds)
for i in range(len(quadratic_CUATRO_global_list2)):
    x_best = np.array(quadratic_CUATRO_global_list2[i]['x_best_so_far'])
    f_best = np.array(quadratic_CUATRO_global_list2[i]['f_best_so_far'])
    x_ind = np.array(quadratic_CUATRO_global_list2[i]['samples_at_iteration'])
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(oracle, bounds)
for i in range(len(quadratic_CUATRO_local_list12)):
    x_best = np.array(quadratic_CUATRO_local_list12[i]['x_best_so_far'])
    f_best = np.array(quadratic_CUATRO_local_list12[i]['f_best_so_far'])
    x_ind = np.array(quadratic_CUATRO_local_list12[i]['samples_at_iteration'])
    ax1.step(x_ind, f_best, where = 'post', label = 'CUATRO_l'+str(i))
    # ax1.plot(x_ind, f_best, label = 'CUATRO_l'+str(i))
    ax2.plot(x_best[:,0], x_best[:,1], '--', label = 'CUATRO_l'+str(i))
ax1.legend()
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(oracle, bounds)
for i in range(len(quadratic_Bayes_list2)):
    x_best = np.array(quadratic_Bayes_list2[i]['x_best_so_far'])
    f_best = np.array(quadratic_Bayes_list2[i]['f_best_so_far'])
    nbr_feval = len(quadratic_Bayes_list2[i]['f_store'])
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(oracle, bounds)
for i in range(len(quadratic_simplex_list2)):
    x_best = np.array(quadratic_simplex_list2[i]['x_best_so_far'])
    f_best = np.array(quadratic_simplex_list2[i]['f_best_so_far'])
    x_ind = np.array(quadratic_simplex_list2[i]['samples_at_iteration'])
...
ax2 = trust_fig(oracle, bounds)
for i in range(N):
    x_best = np.array(quadratic_Nest_list2[i]['x_best_so_far'])
    f_best = np.array(quadratic_Nest_list2[i]['f_best_so_far'])
    x_ind = np.array(quadratic_Nest_list2[i]['samples_at_iteration'])
