...
RTO_pybobyqa2 = PyBobyqaWrapper().solve(RTO_rand, x0, bounds=bounds.T, \
...
for i in range(N):
    rnd_seed = i
    RTO_Nest = nesterov_random(RTO_rand, x0, bounds, max_iter = 50, \
                               constraints = 2, rnd_seed = i, alpha = 1e-4)
    RTO_Nest_list2.append(RTO_Nest)
...
for i in range(N):
    rnd_seed = i
    RTO_simplex = simplex_method(RTO_rand, x0, bounds, max_iter = 50, \
                            constraints = 2, rnd_seed = i, mu_con = 1e6)
    RTO_simplex_list2.append(RTO_simplex)
print('10 simplex iterations completed')
# print(x0)

RTO_FiniteDiff2 = finite_Diff_Newton(RTO_rand, x0, bounds = bounds, \
                                   con_weight = 100)
# print(x0)  
# RTO_BFGS = BFGS_optimizer(RTO, x0, bounds = bounds, \
#                          con_weight = 100)
    
RTO_Adam2 = Adam_optimizer(RTO_rand, x0, method = 'forward', \
...
N = 10
RTO_CUATRO_global_list2 = []
for i in range(N):
    rnd_seed = i
    RTO_CUATRO_global = CUATRO(RTO_rand, x0, init_radius, bounds = bounds, \
                          N_min_samples = N_min_s, tolerance = 1e-10,\
                          beta_red = 0.9, rnd = rnd_seed, method = 'global', \
                          constr_handling = method)
    RTO_CUATRO_global_list2.append(RTO_CUATRO_global)
...
method = 'Fitting'
N = 10
RTO_CUATRO_local_list2 = []
for i in range(N):
    rnd_seed = i
    RTO_CUATRO_local = CUATRO(RTO_rand, x0, init_radius, bounds = bounds, \
                          N_min_samples = N_min_s, tolerance = 1e-10,\
                          beta_red = 0.9, rnd = rnd_seed, method = 'local', \
                          constr_handling = method)
    RTO_CUATRO_local_list2.append(RTO_CUATRO_local)
...
for i in range(1):
    ...
    RTO_Bayes = Bayes.solve(RTO_rand, x0, acquisition='EI',bounds=bounds.T, \
                            print_iteration = True, constraints=2, casadi=True, \
                            maxfun = nbr_feval, ).output_dict
    RTO_Bayes_list2.append(RTO_Bayes)
 
print('10 BayesOpt iterations completed')

with open('BayesRTO_listRand.pickle', 'wb') as handle:
    pickle.dump(RTO_Bayes_list2, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
x_best_pyBbyqa = np.array(RTO_pybobyqa2['x_best_so_far'])
f_best_pyBbyqa = np.array(RTO_pybobyqa2['f_best_so_far'])
# x_ind_pyBbyqa = np.array(RB_pybobyqa['samples_at_iteration'])
# nbr_feval_pyBbyqa = len(RB_pybobyqa['f_store'])

x_best_finDiff = np.array(RTO_FiniteDiff2['x_best_so_far'])
f_best_finDiff = np.array(RTO_FiniteDiff2['f_best_so_far'])
x_ind_findDiff = np.array(RTO_FiniteDiff2['samples_at_iteration'])
...
x_best_Adam = np.array(RTO_Adam2['x_best_so_far'])
f_best_Adam = np.array(RTO_Adam2['f_best_so_far'])
x_ind_Adam = np.array(RTO_Adam2['samples_at_iteration'])
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(X, Y, Z, g1, g2)
for i in range(len(RTO_CUATRO_global_list2)):
    x_best = np.array(RTO_CUATRO_global_list2[i]['x_best_so_far'])
    f_best = np.array(RTO_CUATRO_global_list2[i]['f_best_so_far'])
    x_ind = np.array(RTO_CUATRO_global_list2[i]['samples_at_iteration'])
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(X, Y, Z, g1, g2)
for i in range(len(RTO_CUATRO_local_list2)):
    x_best = np.array(RTO_CUATRO_local_list2[i]['x_best_so_far'])
    f_best = np.array(RTO_CUATRO_local_list2[i]['f_best_so_far'])
    x_ind = np.array(RTO_CUATRO_local_list2[i]['samples_at_iteration'])
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(X, Y, Z, g1, g2)
for i in range(len(RTO_Bayes_list2)):
    x_best = np.array(RTO_Bayes_list2[i]['x_best_so_far'])
    f_best = np.array(RTO_Bayes_list2[i]['f_best_so_far'])
    nbr_feval = len(RTO_Bayes_list2[i]['f_store'])
...
ax1 = fig1.add_subplot()
ax2 = trust_fig(X, Y, Z, g1, g2)
for i in range(len(RTO_simplex_list2)):
    x_best = np.array(RTO_simplex_list2[i]['x_best_so_far'])
    f_best = np.array(RTO_simplex_list2[i]['f_best_so_far'])
    x_ind = np.array(RTO_simplex_list2[i]['samples_at_iteration'])
...
ax2 = trust_fig(X, Y, Z, g1, g2)
for i in range(N):
    x_best = np.array(RTO_Nest_list2[i]['x_best_so_far'])
    f_best = np.array(RTO_Nest_list2[i]['f_best_so_far'])
    x_ind = np.array(RTO_Nest_list2[i]['samples_at_iteration'])
